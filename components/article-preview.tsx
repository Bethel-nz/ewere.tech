...
export function ArticlePreview({ title, date, slug, content, image, showArrow = true }: ArticlePreviewProps) {
	return (
		...
		>
			<div className="group">
				<Link href={`/articles/${slug}`} className="flex gap-6 items-start">
					...
					<div className="flex-1 min-w-0">
						...
						{showArrow && (
							<div className="mt-2 text-primary opacity-0 -translate-x-2 transition-all group-hover:opacity-100 group-hover:translate-x-0">
								Read more →
							</div>
						)}
					</div>
